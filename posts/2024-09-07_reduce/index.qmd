...
We could also write a loop for the sum, which does exactly the same operations as `reduce()`, but in a more complicated way.
...
We usually compute it with `var(my_vec) =` `r var(my_vec)`. 
This is an unbiased estimator of the variance because the denominator is `n-1 = length(my_vec) - 1 =` `r length(my_vec) - 1`. 
...
The function `reduce2()` takes as arguments a first vector and a second one which is shorter than the former by one element, if `.init` is not specified. 
Otherwise, both arguments should have  the same number of of elements.
...
The `reduce2()` function takes as the initial value the first element of the first argument, if `.init` is not specified. 
Otherwise, the argument `.init` gives the first element. 
...
You can alsow use `accumlate2()` if you use 2 lists.
