...
Usually, we compute it with `var(my_vec) =` `r var(my_vec)`. This is an unbiased estimator of the variance because the denominator is `n-1 = length(my_vec) - 1 =` `r length(my_vec) - 1`. This corresponds to the following operation:
...
$$
However, we do not get the same result when we try to apply this formula using `reduce()`, because we need to supply two arguments. 
```{r}
#| error: true
...
my_vec |>
  purrr::reduce(\(x, y) 1 / (length(c(x, y)) - 1) * sum((c(x, y) - mean(x, y))^2))
```
...
$$
Thus, we get `0` because `reduce()` does the following operation: $h(h(h(1, 2), 3), 4)$ which is equal to 
