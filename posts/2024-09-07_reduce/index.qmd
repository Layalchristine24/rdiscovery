...
Usually, we compute it with `var(my_vec) =` `r var(my_vec)`. This is an unbiased estimator of the variance because the denominator is `n-1 = length(my_vec) - 1 =` `r length(my_vec) - 1`. The following operation:
...
$$
Thus, we get `r purrr::reduce(my_vec, \(x, y) var(c(x, y)))` because `reduce()` does the following operation: $h(h(h(1, 2), 3), 4)$ which is equal to 
