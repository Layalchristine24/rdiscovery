...
*When should `&` and `|` rather than `&&` or `||` be used?*

I would like to make clear when to use `&` and when to use `&&` in R. I will also explain the difference between `|` and `||`.

# `&` and `&&`

The `&` operator is used to combine two logical vectors and returns a logical vector of the same length as the two input vectors. The `&&` operator is used to combine two logical vectors and returns a single logical value. Let's assume the following objects.
