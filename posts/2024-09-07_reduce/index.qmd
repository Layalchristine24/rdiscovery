...
Let's say we want to apply various various operations to our vector elements.

```{r}
my_vec |>
  purrr::reduce(\(x, y) mean((x-y)^2))

var(my_vec)

my_vec |>
  purrr::reduce(\(x, y) mean(x^2) - mean(y)^2)
```

Please note that these operations do not return the variance of `my_vec` because 
