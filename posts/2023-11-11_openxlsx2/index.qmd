...
In this example, we will do the following improvements:

- Add a filter to the two first columns, namely `species` and `island`

- Set the first active row to the one which contains data, juste below the column names, and the first active column to the `island` column (the second one)

- Protect the second worksheet, namely "my_second_sheet"  
...
wb_pff <- wb_cf$
  ...
  first_active_col = start_col_ws1 +
    which(names(penguins) == "island")
)$
  protect_worksheet(
  sheet = "my_second_sheet"
